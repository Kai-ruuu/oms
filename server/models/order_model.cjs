class Order {
   ...
   getBySellerID(sellerID) {
      return this.safeCall(this.collection().getFullList({ filter: `seller='${sellerID}'`, sort: "-created" }))
   }

   getByBuyerID(buyerID) {
      return this.safeCall(this.collection().getFullList({ filter: `buyer='${buyerID}'`, sort: "-created" }))
   }
